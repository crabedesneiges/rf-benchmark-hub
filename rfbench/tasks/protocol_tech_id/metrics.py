"""Pure-stdlib protocol-tech-ID metrics.

Protocol-tech-ID (WiFi 802.11 standard recognition) is a single-label closed-set
classification task with no SNR grid, so it reuses the task-agnostic AMC classification
machinery but WITHOUT the AMC-specific ``full_snr_range`` eval condition. Per
``docs/EVALUATION_PROTOCOL.md`` §protocol_tech_id the reported metrics are:
...
class AccuracyOverall(Metric):
    ...
    object fills the two mandatory protocol-tech-ID scalars.

    Unlike :class:`rfbench.tasks.amc.metrics.AccuracyOverall`, this metric declares NO
    ``eval_conditions``: protocol-tech-ID has no SNR grid, so nothing extra is written to
