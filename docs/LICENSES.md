...
`docs/BIBLIOGRAPHY.md`, `docs/SOTA_REFERENCE.md`, and the download-script docstrings under
`rfbench/data/download/` — nothing below is a new claim; where a source states the license is
unconfirmed, this table says so explicitly rather than guessing.
...
| RadioML 2018.01a | O'Shea, Roy, Clancy, *IEEE JSTSP* 2018, arXiv:1712.04578, DOI 10.1109/JSTSP.2018.2797022 | **CC BY-NC-SA 4.0** (same DeepSig terms as 2016.10a, per `rfbench/data/download/amc_radioml.py`) | Nothing (D3) — dataset currently **blocked** on the cluster (Kaggle-gated / DeepSig cert expired), so no split has been generated yet | https://opendata.deepsig.io/datasets/2018.01/ |
...
| ORACLE | Sankhe, Rajendran, Belgiovine, Chowdhury, Ioannidis, *IEEE INFOCOM 2019*, arXiv:1812.01124, DOI 10.1109/INFOCOM.2019.8737463 | **Not confirmed** — `rfbench/data/download/sei_oracle.py` documents provenance (Northeastern/Genesys Lab, SigMF captures) and the "never redistribute" rule but does not state an explicit licence; treat as unconfirmed until checked on the Northeastern DRS landing page | Nothing (D3); dataset is downloadable (valid cert) but **not yet fetched** on the cluster — no split generated yet | https://genesys-lab.org/oracle |
